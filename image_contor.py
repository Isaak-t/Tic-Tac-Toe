...
def loop_img(my_image):
    ...
    my_image.show()
messupimage = IntVar()
Checkbutton(root, text='messed up image', variable=messupimage).grid(row=7, column=5, sticky=W)
def Contor():
    global pic
    filetypes = (('image files', '*.jpg *.png'), ('All files', '*.*'))
    #file_name = fd.askopenfilename(title='Open files', initialdir='/', filetypes=filetypes)
...
def Emboss():
    ...
    filetypes = (('image files', '*.jpg *.png'), ('All files', '*.*'))
    #file_name = fd.askopenfilename(title='Open files', initialdir='/', filetypes=filetypes)
   # pick = Image.open(file_name)
    pick = pick.filter(ImageFilter.EMBOSS)
    pick.show()
...
embossbutton1 = Checkbutton(root, onvalue=1, offvalue=0, text='emboss image', command=Emboss)
embossbutton1.grid(row=7,column=4)
contourbutton1 = Checkbutton(root, onvalue=1, offvalue=0, text='contour image', command=Contor)
contourbutton1.grid(row=7, column=3)
